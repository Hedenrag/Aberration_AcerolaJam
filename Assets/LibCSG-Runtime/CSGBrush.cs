...
namespace LibCSG
{
    ...
    **/
    public class CSGBrush
    {
        ...
        /// <summary>
        /// Update the matrix of the <c>Object</c>. Needed if the transform has changed
        /// </summary>
        ...
        public CSGBrush(GameObject objet)
        {
            faces = new Face[0];
            obj = objet;
            transformMatrix = objet.transform.localToWorldMatrix;
            localScale = obj.transform.localScale;
        }
        ...
        public void regen_face_aabbs()
        {
            for (int i = 0; i < faces.Length; i++)
            {
                ...
                faces[i].aabb.set_position(transformMatrix.MultiplyPoint(faces[i].vertices[0]));
                faces[i].aabb.expand_to(transformMatrix.MultiplyPoint(faces[i].vertices[1]));
                faces[i].aabb.expand_to(transformMatrix.MultiplyPoint(faces[i].vertices[2]));
            }
        }
        ...
        public FakeMesh getAsyncMesh()
        {
            FakeMesh m = new FakeMesh();
            Vector3[] vert = new Vector3[faces.Length * 3];
            Vector2[] uv = new Vector2[faces.Length * 3];
            int[] triangles = new int[faces.Length * 3];
            for (int i = 0; i < faces.Length; i++)
