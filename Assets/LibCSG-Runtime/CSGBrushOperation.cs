...
namespace LibCSG
{
    ...
    **/
    public class CSGBrushOperation
    {
        ...
        {
            build2DFaceCollection = new Build2DFaceCollection();
            build2DFaceCollection.build2DFacesA = new();
            build2DFaceCollection.build2DFacesB = new();
        }
        ...
        {
            build2DFaceCollection.build2DFacesA.Clear();
            build2DFaceCollection.build2DFacesA.EnsureCapacity(brush_a.faces.Length);
            build2DFaceCollection.build2DFacesB.Clear();
            build2DFaceCollection.build2DFacesB.EnsureCapacity(brush_b.faces.Length);
            ...
            //mesh_merge.scale_a = brush_a.obj.transform.localScale; Original 
            mesh_merge.scale_a = brush_a.localScale;
            ...
            for (int i = 0; i < brush_b.faces.Length; i++)
            {
                ...
                else
                {
                    ...
                    for (int j = 0; j < 3; j++)
                    {
                        ...
                        points[j] = brush_a.transformMatrix.inverse.MultiplyPoint(brush_b.transformMatrix.MultiplyPoint(brush_b.faces[i].vertices[j]));
        ...
        void update_faces(ref CSGBrush brush_a, int face_idx_a, ref CSGBrush brush_b, int face_idx_b, ref Build2DFaceCollection collection, float vertex_snap)
        {
            ...
            brush_a.transformMatrix.inverse.MultiplyPoint(brush_b.transformMatrix.MultiplyPoint(brush_b.faces[face_idx_b].vertices[0])),
            brush_a.transformMatrix.inverse.MultiplyPoint(brush_b.transformMatrix.MultiplyPoint(brush_b.faces[face_idx_b].vertices[1])),
            brush_a.transformMatrix.inverse.MultiplyPoint(brush_b.transformMatrix.MultiplyPoint(brush_b.faces[face_idx_b].vertices[2]))
        };
